...
def routine(W1_dim=[5,5,1,32],b1_dim=[32],W2_dim=[5, 5, 32, 64],b2_dim=[64]):
    ...
    h_pool2_flat = tf.reshape(h_pool2, [-1, 7*7*b2_dim[0]])
...
def main():
    for i in range(0,8):
        for j in range(0,8):
            for (n1,n2) in [(2**i,2**j)]:
                routine(W1_dim=[5,5,1,n1],b1_dim=[n1],W2_dim=[5, 5, n1, n2],b2_dim=[n2])
                print("number of channels in the first layer is %d\nnumber of channles in the second layer is %d" %(n1,n2))
